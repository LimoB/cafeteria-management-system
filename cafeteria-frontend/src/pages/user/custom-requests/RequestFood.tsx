...
const RequestFood: React.FC = () => {
  ...
  // Memoize activeUser - priority to fresh fetched data
  ...
  const handlePayment = async (orderId: string) => {
    ...
    if (!orderToPay || !orderToPay.price) {
      toast.error("Price not assigned yet.");
      return;
    }

    const userFromList = users.find(u => String(u.id) === String(authUser?.id));
    const phone = userFromList?.phone || (selectedUser as any)?.phone || (authUser as any)?.phone || (activeUser as any)?.phone;

    if (!phone || String(phone).trim() === '') {
      toast.error("Phone number missing. Check your profile settings.");
      return;
    }
    ...
    try {
      ...
      toast.success(`STK Push sent to ${cleanPhone}`);
    } catch (err: any) {
  ...
  const getStatusStyle = (status: string | undefined) => {
    ...
    switch (s) {
      case 'approved': return { bg: 'bg-emerald-500/10', text: 'text-emerald-600', icon: <CheckCircle2 size={12} /> };
      case 'rejected': return { bg: 'bg-red-500/10', text: 'text-red-600', icon: <XCircle size={12} /> };
      case 'pending': return { bg: 'bg-amber-500/10', text: 'text-amber-600', icon: <Clock size={12} /> };
      default: return { bg: 'bg-slate-500/10', text: 'text-slate-600', icon: <Info size={12} /> };
    }
  };
          ...
          <div className="bg-white border border-gray-100 rounded-[3rem] p-8 md:p-12 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  rows={4}
                  placeholder="Tell the chef exactly what you need..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full p-5 bg-slate-50 border border-gray-100 rounded-2xl outline-none focus:bg-white focus:border-red-200 font-medium resize-none"
                />
              </div>
              ...
              <button 
                ...
                className="w-full bg-slate-950 hover:bg-red-600 text-white font-black uppercase tracking-widest py-6 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-xl disabled:opacity-50"
              >
        ...
        <div className="space-y-6">
          <div className="bg-white border border-gray-100 rounded-[3rem] p-8 shadow-sm h-full">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 flex items-center gap-3 mb-8">
              <HistoryIcon size={16} className="text-red-600" /> Activity Log
            </h3>
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {(!customOrders || customOrders.length === 0) ? (
                ...
                customOrders.map((order) => {
                  ...
                  const isPendingPayment = order.approvalStatus === 'approved' && order.paymentStatus !== 'completed';
                  
                  const userInList = users.find(u => String(u.id) === String(authUser?.id));
                  const displayPhone = userInList?.phone || (authUser as any)?.phone || (activeUser as any)?.phone;

                  return (
                    <div key={order.id} className="p-6 bg-slate-50 rounded-[2.5rem] border border-gray-100 transition-all hover:border-red-100 group">
                      <div className="flex justify-between items-start mb-4">
                        <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2 ${style.bg} ${style.text}`}>
                          {style.icon} {order.approvalStatus}
                        </div>
                      ...
                      <div className="flex flex-col gap-3 pt-4 border-t border-gray-200/50">
                        {order.approvalStatus === 'approved' && (
                          <div className="space-y-3">
                            <div className="flex items-center justify-between px-3 py-2 bg-white/50 rounded-xl border border-slate-100 shadow-sm">
                                <div className="flex items-center gap-2">
                                    <Phone size={10} className="text-slate-400" />
                                    <span className="text-[8px] font-black uppercase text-slate-400 tracking-wider">M-Pesa Target</span>
                                </div>
                                <span className={`text-[10px] font-bold ${displayPhone ? 'text-slate-700' : 'text-red-500'}`}>
                                  {displayPhone || 'Phone Missing'}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                               <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-sm font-black text-emerald-600">KES {order.price}</span>
                               </div>
                               {isPendingPayment ? (
                                 <button 
                                   type="button"
                                   ...
                                   className="bg-slate-950 text-white text-[9px] font-black px-4 py-2 rounded-xl uppercase tracking-widest hover:bg-red-600 transition-all flex items-center gap-2 shadow-lg disabled:opacity-50"
                                  >
                                   {isPaying === order.id ? <Loader2 size={10} className="animate-spin" /> : <CreditCard size={12} />}
                                   Pay Now
                                 </button>
                               ) : (
                                 <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest bg-emerald-100 px-3 py-1 rounded-lg">Paid</span>
                               )}
                            </div>
                          </div>
                        )}
